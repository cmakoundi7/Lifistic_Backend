...
​
	...
	// strtolowe : Renvoie une chaîne en minuscules
	// ********************************************************************************
	...
	$res = standardQuery("whrite", $sql, $endpoint);
	...
	$sql = sprintf("UPDATE LIFISTIC_1.LIFISTIC_USERS 
			SET DATA_S = JSON_SET(DATA_S, '$.\"pending_invites\"', CAST('$pending_invitations' AS JSON)) 
			WHERE USER_S = '$userid' AND STATUS_ID = 1)"; 

	$res = standardQuery("white", $sql, $endpoint);
	unset($sql);
	// ********************************************************************************
	// WRAP & SEND	
